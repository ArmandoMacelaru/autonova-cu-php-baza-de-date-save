...
if ($db_error) {
    ...
    if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
        ...
        if (isset($_SESSION['user_id'])) {
            if ($stmtF = $conn->prepare("SELECT 1 FROM favorite WHERE user_id=? AND masina_id=? LIMIT 1")) {
                $uid = (int)$_SESSION['user_id'];
                $stmtF->bind_param("ii", $uid, $id);
                $stmtF->execute();
                $is_fav = ($stmtF->get_result()->num_rows > 0);
                $stmtF->close();
            }
        }
        ...
        if ($masinaRes->num_rows === 0) {
            ...
            <div class="car-detail-card">
                <div class="car-detail-header">
                    ...
                    <?php endif; ?>
                    <?php $heart = ($is_fav ?? false) ? "❤" : "♥"; ?>
                    <button class="favorite-btn-large" type="button" id="favBtn" data-masina-id="<?php echo (int)$id; ?>"><?php echo $heart; ?></button>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (favBtn) {
    ...
    // inițial (din PHP)
    setFavUI(favBtn.textContent.trim() === '❤');

    favBtn.addEventListener('click', async () => {
      const masinaId = favBtn.getAttribute('data-masina-id');
      if (!masinaId) return;
      ...
      try {
        const res = await fetch('favorite_toggle.php', {
          ...
          body: new URLSearchParams({ masina_id: masinaId })
        });
        ...
        if (!data.ok) {
          alert(data.error || 'Eroare la favorite.');
          return;
        }

        setFavUI(!!data.favorit);
      } catch (e) {
