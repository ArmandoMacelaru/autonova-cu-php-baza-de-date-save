...
$isLoggedIn = false;
if (isset($_SESSION['user_id']) || isset($_COOKIE['autonova_user'])) {
    $isLoggedIn = true;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $marca = $conn->real_escape_string($_POST['make']);
    $model = $conn->real_escape_string($_POST['model']);
    $an = intval($_POST['year']);
    $pret = intval($_POST['price']);
    $combustibil = $conn->real_escape_string($_POST['fuel']);
    $cutie_viteze = $conn->real_escape_string($_POST['transmission']);
    $kilometraj = intval($_POST['mileage']);
    $culoare = $conn->real_escape_string($_POST['color']);
    $descriere = $conn->real_escape_string($_POST['description']);
    $contact = $conn->real_escape_string($_POST['contact']);
    
    // Determină vânzătorul
    $seller = "Persoană fizică";
    if ($isLoggedIn && isset($_SESSION['username'])) {
        $seller = $_SESSION['username'];
    } elseif ($isLoggedIn && isset($_COOKIE['autonova_user'])) {
        $user_data = json_decode($_COOKIE['autonova_user'], true);
        $seller = $user_data['username'];
    }
    
    // Imagine default sau cea furnizată
    $imagine = "https://images.unsplash.com/photo-1583121274602-3e2820c69888?w=800&h=450&fit=crop";
    if (!empty($_POST['images'])) {
        $imagini = explode(',', $_POST['images']);
        $imagine = trim($imagini[0]); // Prima imagine este cea principală
    }
    ...
    $stmt->bind_param("ssiiisssss", 
        $marca, $model, $an, $pret, $kilometraj, 
        $combustibil, $cutie_viteze, $culoare, $descriere, $imagine
    );
    
    if ($stmt->execute()) {
        $masina_id = $conn->insert_id;
        $success_message = "✅ Anunțul tău a fost publicat cu succes! ID mașină: #$masina_id";
        
        // Adaugă imagini extra în tabelul imagini_masini
        if (!empty($_POST['images'])) {
            $imagini = explode(',', $_POST['images']);
            $ordine = 1;
            foreach ($imagini as $img_url) {
                ...
                if (!empty($img_url)) {
                    ...
                    $sql_img = "INSERT INTO imagini_masini (masina_id, url_imagine, descriere, este_principala, ordine) 
                               VALUES (?, ?, 'Imagine anunț', ?, ?)";
                    $stmt_img = $conn->prepare($sql_img);
                    $stmt_img->bind_param("isii", $masina_id, $img_url, $este_principala, $ordine);
                    $stmt_img->execute();
                    $ordine++;
                }
            }
        ...
        $error_message = "❌ Eroare la publicarea anunțului: " . $stmt->error;
    }
        ...
        <div class="form-container">
            <?php if (!$isLoggedIn): ?>
                <div class="message info">
                    ...
                    <a href="login.html" style="color: #1565c0; font-weight: bold;">conectează-te</a> 
                    sau 
                    <a href="register.html" style="color: #1565c0; font-weight: bold;">înregistrează-te</a>.
                    Poți publica anunț și fără cont.
                </div>
            <?php endif; ?>
            
            <form id="add-car-form" method="POST" action="">
                <div class="form-row">
                    <div class="form-col">
